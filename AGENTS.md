...
Linux/WSL에서 독립 ORCA 작업을 durable queue와 supervised worker로 실행한다.
공개 계약은 [docs/PUBLIC_CONTRACTS.md](docs/PUBLIC_CONTRACTS.md)가 정본이다.
...
| 범위 | 문서 |
| --- | --- |
| CLI·설정·상태·복구 계약 | [PUBLIC_CONTRACTS](docs/PUBLIC_CONTRACTS.md) |
...
`machine.json`의 공통 v1 봉투는 `~/machine_contracts/COMPATIBILITY.md`를 따른다.
공통 계약 변경은 해당 저장소에 먼저 반영하고 CI pin을 갱신한다.
`job_state.json`은 내부 복구 파일이다.

## 운영 경계

- 검증·패키지 게시·운영 전환은 별도 단계다.
...
- 실제 ORCA 실행 의미가 바뀌면 bounded real-engine acceptance가 필요하다.
- 7.0은 워크플로우 기능을 제거했다. 과거 상태의 읽기·소유권 판별과 실행 지원을
