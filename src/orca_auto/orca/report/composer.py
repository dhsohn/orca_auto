...
def compose_job_report_html(reaction_dir: Path, state: Mapping[str, Any]) -> str | None:
    ...
    fallback_reason = _fallback_reason(primary)

    page = ReportPage(
        title=f"{_title(primary)} · {_kind_label(parts, primary)} report",
        badges=badges,
        meta_html=_meta_html(parts, primary),
        verdict_html=verdict_note(_reason(primary), fallback_reason),
        metrics_html=metrics_html,
        sections=sections,
        footer_html=(
            "Generated by orca_auto &#183; last output: "
            f"<code>{html.escape(_last_out_name(primary))}</code>"
        ),
    )
...
def _primary_report_data(parts: HtmlReportParts) -> object | None:
    # The primary object supplies the page title, status, footer, and the one
...
def _kind_label(parts: HtmlReportParts, primary: object) -> str:
    ...
    if isinstance(primary, OptReportData):
        return primary.kind_label()
    if isinstance(primary, SpReportData):
        return "SP"
    return "Job"


def _combined_badges(parts: HtmlReportParts, primary: object) -> tuple[tuple[str, str], ...]:
    if isinstance(primary, NebReportData):
    ...
    elif isinstance(primary, SpReportData):
...
def _meta_html(parts: HtmlReportParts, primary: object) -> str:
    if isinstance(primary, NebReportData):
    ...
    route = file_route_lines(parts.selected_inp)
    return f"<code>{html.escape(route[0] if route else parts.routes)}</code>"


def _title(data: object) -> str:
    return str(getattr(data, "title", "Job"))


def _reason(data: object) -> str:
    return str(getattr(data, "reason", ""))


def _last_out_name(data: object) -> str:
    return str(getattr(data, "last_out_name", ""))


def _fallback_reason(data: object) -> str:
    attempts = getattr(data, "attempts", ())
    if not attempts:
        return ""
    return str(attempts[-1].analyzer_reason)
