...
def render_neb_report_html(data: NebReportData) -> str:
    fallback_reason = data.attempts[-1].analyzer_reason if data.attempts else ""
    component = neb_report_component(data)

    page = ReportPage(
        title=f"{data.title} · NEB-TS report",
        badges=neb_report_badges(data),
        meta_html=neb_report_meta_html(data),
