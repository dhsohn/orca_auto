...
from .orca_process import ORCA_PROCESS_RECORD_FILE_NAME, ORCA_PROCESS_RECORD_LOCK_FILE_NAME
...
_SCRATCH_CONTROL_FILE_NAMES = frozenset(
    {
        ...
        ORCA_PROCESS_RECORD_LOCK_FILE_NAME,
    }
)
...
class OrcaScratchPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if root == parent or not root.is_relative_to(parent):
            raise ValueError("ORCA scratch root must be a dedicated directory below /dev/shm")
        if self.min_free_bytes < 1:
            raise ValueError("ORCA scratch minimum free bytes must be positive")
        if self.max_task_memory_bytes < 1:
            raise ValueError("ORCA task memory limit must be positive")
...
def _prepare_scratch_root(policy: OrcaScratchPolicy) -> Path:
...
def _capture_input_closure(
    ...
    try:
        lines = selected_payload.decode("utf-8", errors="strict").splitlines()
    except UnicodeError as exc:
        raise OrcaScratchError(
            f"ORCA input is not UTF-8 text: {durable_dir / selected_name}"
        ) from exc
    captured = {
    ...
    for reference in scan_orca_file_references(lines):
        raw = Path(reference.value)
        if raw.is_absolute() or raw.name != reference.value or raw.name in {"", ".", ".."}:
            raise OrcaScratchError(
                "ORCA RAM scratch requires input dependencies to use one relative basename: "
                f"{reference.value!r}"
            )
...
def _input_closure_size_bytes(
    ...
    if selected_payload and not selected_payload.endswith(b"\n"):
        required += 1
    return required
...
def _stage_input_closure(
    ...
    *,
    ...
    for name, captured in captured_inputs.items():
        ...
        if source == durable_input and payload and not payload.endswith(b"\n"):
            staged_payload += b"\n"
        item = _StagedInput(
            source=source,
            name=name,
...
def _scratch_entries(workspace: Path, workspace_dir_fd: int) -> list[str]:
    ...
    for name in sorted(os.listdir(workspace_dir_fd)):
        ...
        if name in _SCRATCH_CONTROL_FILE_NAMES:
            continue
        info = os.stat(name, dir_fd=workspace_dir_fd, follow_symlinks=False)
        if not stat.S_ISREG(info.st_mode):
            raise OrcaScratchError(
                f"ORCA scratch produced an unsupported entry: {workspace / name}"
            )
        entries.append(name)
    return entries
