...
severity) and the author the embed author line. Discord's documented limits are
enforced by truncation so a large notification degrades gracefully instead of
being rejected.
"""
...
from .richtext import Field, Message, Span
...
def _truncate(value: str, limit: int) -> str:
    ...
    backslashes. Only field values (which do parse Markdown) are escaped.
    """
    if len(value) <= limit:
        return value
    if limit <= 1:
...
def _md_spans(spans: tuple[Span, ...], *, limit: int | None = None) -> str:
    ...
    # Span boundaries (or within one Span) so code delimiters stay closed.
    budget = limit - 1
    output: list[str] = []
...
def render_discord_embed(message: Message) -> dict[str, Any]:
    ...
    fields = [item for group in message.groups for item in group.items]
