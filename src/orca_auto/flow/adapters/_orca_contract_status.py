...
def attempt_count_impl(state: dict[str, Any], report: dict[str, Any]) -> int:
...
def max_retries_impl(state: dict[str, Any], report: dict[str, Any]) -> int:
...
def status_payload(
    *,
    ...
    queue_status = normalize_text(queue.get("status")).lower()
    cancel_requested = normalize_bool(queue.get("cancel_requested"))
