...
def test_queue_compaction_roots_reads_configured_admission_root(tmp_path: Path) -> None:
    ...
    assert unified_cli._queue_compaction_roots(
        SimpleNamespace(
            orca_auto_config=str(config_path),
            config=None,
            workflow_root=str(runs_root),
        )
    ) == (runs_root.resolve(), admission_root.resolve())


def _queue_compaction_result(**overrides: Any) -> SimpleNamespace:
    values: dict[str, Any] = {
        "workflow_id": "wf-compact-1",
        "workspace_dir": Path("/tmp/workflows/wf-compact-1"),
        "eligible": True,
        "blocked": False,
        "applied": False,
        "would_remove": (
            SimpleNamespace(
                relative_path="01_crest/run/job_report.json",
                size=41,
                engine="crest",
                stage_id="01_crest",
                job_id="crest-1",
            ...
            SimpleNamespace(
                relative_path="02_xtb/run/job_report.md",
                size=17,
                engine="xtb",
                stage_id="02_xtb",
                job_id="xtb-1",
        ...
        "removed": (),
        "removed_bytes": 0,
        "preserved": ("workflow.json", "02_xtb/run/job_state.json"),
        "reasons": (),
    }
...
def test_cmd_queue_compact_dry_run_json_is_non_mutating_and_stable(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    calls: list[tuple[Path, Path, bool, Path]] = []
    workflow_root = Path("/tmp/workflows")
    admission_root = Path("/tmp/admission")
    workspace = Path("/tmp/workflows/wf-compact-1")
    monkeypatch.setattr(
        unified_cli,
        "_queue_compaction_roots",
        lambda args: (workflow_root, admission_root),
    )
    monkeypatch.setattr(
        unified_cli,
        "resolve_workflow_workspace",
        lambda *, target, workflow_root: workspace,
    )
    ...
    def fake_compact(
        ...
        *,
        ...
        calls.append((resolved_root, workspace_dir, apply, admission_root))
        return _queue_compaction_result()

    monkeypatch.setattr(unified_cli, "compact_completed_workflow", fake_compact)

    result = unified_cli.cmd_queue_compact(
        SimpleNamespace(
            target="wf-compact-1",
            workflow_root=None,
            orca_auto_config="/tmp/orca_auto.yaml",
    ...
    assert result == 0
    assert calls == [(workflow_root, workspace, False, admission_root)]
    assert json.loads(capsys.readouterr().out) == {
        "workflow_id": "wf-compact-1",
        "workspace_dir": "/tmp/workflows/wf-compact-1",
        "eligible": True,
        "blocked": False,
        "applied": False,
        "would_remove": [
...
def test_cmd_queue_compact_apply_reports_removed_bytes(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    workspace = Path("/tmp/workflows/wf-compact-1")
    monkeypatch.setattr(
        unified_cli,
        "_queue_compaction_roots",
        lambda args: (Path("/tmp/workflows"), Path("/tmp/admission")),
    )
    monkeypatch.setattr(
        unified_cli,
        "resolve_workflow_workspace",
    ...
    monkeypatch.setattr(
        ...
        lambda workflow_root, workspace_dir, *, apply, admission_root: _queue_compaction_result(
            applied=True,
            removed=(
                "01_crest/run/job_report.json",
                "02_xtb/run/job_report.md",
            ),
            removed_bytes=58,
        ),
    )
    ...
    assert "workflow_id: wf-compact-1" in output
    assert "workspace_dir: /tmp/workflows/wf-compact-1" in output
    assert "eligible: yes" in output
    assert "blocked: no" in output
    assert "applied: yes" in output
    assert "removed_bytes: 58" in output


def test_cmd_queue_compact_blocked_result_is_fail_closed(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        unified_cli,
        "resolve_workflow_workspace",
        lambda *, target, workflow_root: Path("/tmp/workflows/wf-running"),
    )
    monkeypatch.setattr(
        unified_cli,
        "compact_completed_workflow",
        lambda workflow_root, workspace_dir, *, apply, admission_root: _queue_compaction_result(
            ...
            blocked=True,
            would_remove=(),
            preserved=("workflow.json",),
            reasons=("workflow is not terminal",),
        ),
    )
    ...
    assert payload["reasons"] == ["workflow is not terminal"]


def test_cmd_queue_compact_apply_without_applied_result_fails_closed(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        unified_cli,
        "resolve_workflow_workspace",
        lambda *, target, workflow_root: Path("/tmp/workflows/wf-compact-1"),
    )
    monkeypatch.setattr(
        unified_cli,
        "compact_completed_workflow",
...
def test_cmd_queue_compact_blocked_partial_apply_reports_actual_removal(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        ...
        lambda workflow_root, workspace_dir, *, apply, admission_root: _queue_compaction_result(
            eligible=False,
            blocked=True,
            removed=("01_crest/run/job_report.json",),
            removed_bytes=41,
            reasons=("report identity changed during apply",),
        ),
    )
    ...
    assert payload["removed"] == ["01_crest/run/job_report.json"]
    assert payload["removed_bytes"] == 41


def test_cmd_queue_compact_core_exception_uses_cli_error_style(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        unified_cli,
        "resolve_workflow_workspace",
        lambda *, target, workflow_root: Path("/tmp/workflows/wf-corrupt"),
    )
    ...
    def fail_compaction(
        ...
        *,
        ...
        raise ValueError("workflow workspace is corrupt")

    monkeypatch.setattr(unified_cli, "compact_completed_workflow", fail_compaction)
    ...
    assert captured.out == ""
    assert captured.err == (
        "error: workflow workspace is corrupt\n"
