...
def test_advance_workflow_conformer_runs_append_sequence_and_sets_child_sync_metadata(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    payload: dict[str, Any] = {
        "workflow_id": "wf_conformer_sequence",
        "template_name": "conformer_screening",
        "status": "planned",
        "stages": [
            {
    ...
    def append_orca(current_payload: dict[str, Any], **_kwargs: Any) -> bool:
        calls.append("append_orca")
        current_payload["stages"].append(
            {
    ...
    _patch_advance_operations(
        monkeypatch,
        sync_crest_stage_impl=lambda stage, **kwargs: calls.append("crest"),
        append_crest_orca_stages_impl=append_orca,
        append_interaction_energy_stages_impl=lambda *args, **kwargs: False,
        sync_orca_stage_impl=lambda stage, **kwargs: calls.append("orca:" + stage["stage_id"]),
    ...
    deps = orchestration_services(
        overrides={
            "resolve_workflow_workspace": lambda **kwargs: tmp_path / "wf_conformer_sequence",
            "acquire_workflow_lock": lambda *args, **kwargs: nullcontext(),
            "load_workflow_payload": lambda workspace: payload,
            "now_utc_iso": lambda: "2026-04-19T12:00:00+00:00",
            "write_workflow_payload": lambda workspace, current: writes.append(deepcopy(current)),
            "sync_workflow_registry": lambda *args: None,
        }
    )
    result = orchestration.advance_workflow(
        target="wf_conformer_sequence",
        workflow_root=tmp_path,
        submit_ready=True,
        services=deps,
    )
    ...
    assert calls[:4] == ["crest", "append_orca", "orca:crest_stage_01", "orca:orca_stage_01"]
...
def test_advance_workflow_checkpoints_completed_crest_before_orca_materialization(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    payload: dict[str, Any] = {
        "workflow_id": "wf_conformer_checkpoint",
        "template_name": "conformer_screening",
        "status": "planned",
        "stages": [
            {
    ...
    def sync_crest(stage: dict[str, Any], **_kwargs: Any) -> None:
        stage["status"] = "completed"
        stage["task"]["status"] = "completed"

    def append_orca(current: dict[str, Any], **_kwargs: Any) -> bool:
        assert writes and writes[0]["stages"][0]["status"] == "completed"
    ...
    _patch_advance_operations(
        monkeypatch,
        sync_crest_stage_impl=sync_crest,
        append_crest_orca_stages_impl=append_orca,
        append_interaction_energy_stages_impl=lambda *args, **kwargs: False,
        sync_orca_stage_impl=lambda *args, **kwargs: None,
        _recompute_workflow_status=lambda current: "running",
        workflow_has_active_children_impl=lambda *args, **kwargs: False,
    )
    deps = orchestration_services(
        overrides={
            "resolve_workflow_workspace": lambda **kwargs: tmp_path / "wf_conformer_checkpoint",
            "acquire_workflow_lock": lambda *args, **kwargs: nullcontext(),
            "load_workflow_payload": lambda workspace: payload,
    ...
    orchestration.advance_workflow(
        target="wf_conformer_checkpoint",
        workflow_root=tmp_path,
        submit_ready=True,
        services=deps,
    )
    assert len(writes) >= 3
    assert [stage["stage_id"] for stage in writes[0]["stages"]] == ["crest_conformer_01"]
    first_orca_checkpoint = next(
        index
        for index, snapshot in enumerate(writes)
        if any(stage["stage_id"] == "orca_conformer_01" for stage in snapshot["stages"])
    )
    assert first_orca_checkpoint > 0
    assert all(
        snapshot["stages"][0]["status"] == "completed"
        for snapshot in writes[:first_orca_checkpoint]
    )
