...
@pytest.mark.parametrize("section", ["crest: fast", "orca: 3"])
...
def _conformer_scaffold(workflow_dir: Path) -> None:
...
def test_cmd_run_dir_passes_scaffold_dir_for_generation_workspace(
    ...
    _conformer_scaffold(workflow_dir)
    captured: dict[str, Any] = {}
...
def test_cmd_run_dir_rejects_parenthesized_workflow_name_before_creation(
    ...
    monkeypatch.setattr(cli_run_dir, "create_conformer_screening_workflow", unexpected_create)
...
def test_cmd_run_dir_for_conformer_uses_nested_engine_sections(
    ...
    assert captured["orca_route_line"] == "! custom Opt"
