...
- `orca_auto-runtime@.target`
  - 통합 큐 워커와 Telegram 봇을 위한 권장 결합 런타임 타깃
...
부팅 시 통합 큐 워커와 통합 Telegram 봇을 함께 시작하려면
...
Telegram 봇을 systemd로 관리하고 싶지 않거나 Telegram이 아직 설정되지 않았을 때는
워커 전용 서비스를 사용하세요. Telegram 설정이 비어 있는 동안에는 설치 프로그램이
자동으로 그 모드를 선택합니다.
