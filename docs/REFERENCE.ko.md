...
기본 fake 프로필은 라이선스 엔진 바이너리 없이 ORCA·워크플로우의
성공/fail-closed 시나리오를 실행하고 결과를 보존합니다. 명령은 배치 디렉터리와
...
보관하거나 삭제하세요. 실제 엔진 acceptance는 `--profile real-orca`,
명시적 실행 파일 환경 변수, 공유 production 설정을 사용해야 하는
