...
이 문서는 ORCA_auto의 CLI 명령어, 런타임 모델, 설정 옵션 및 내부 동작 방식을 상세히 설명합니다.

공개 계약으로 보호되는 안정적인 인터페이스 목록은 [공개 계약(PUBLIC_CONTRACTS.ko.md)](PUBLIC_CONTRACTS.ko.md)을 참고하세요.

## 1) 설계 및 동작 원칙

- **격리된 작업 루트**: 지정된 `runs_root` 디렉터리 내에서만 안전하게 작업을 실행합니다.
- **입력 파일 자동 바인딩**: 제출 시 작업 디렉터리에서 가장 최근에 수정된 ORCA 입력 파일(`*.inp`)을 자동으로 감지하여 바인딩합니다.
- **안전한 디스크 큐**: 작업 요청을 디스크(`queue.json`)에 영속적으로 저장한 후 즉시 반환합니다.
- **백그라운드 워커 데몬**: systemd로 관리되는 워커가 큐의 작업을 순차적으로 가져와 실행합니다.
- **무분별한 재시도 방지**: 계산 실패 시 원본 입력을 덮어쓰거나 무분별하게 자동 재실행하지 않고 명확한 실패 기록을 남깁니다.
- **체크포인트 재개 지원**: 비정상 중단된 실행을 재개할 때 유효한 ORCA 바이너리 웨이브펑션 파일(`.gbw`)이 있으면 이를 활용합니다.
- **구조화된 결과 기록**: 실행 상태, 진단 로그 및 결과를 각 작업 디렉터리 내에 투명하게 기록합니다.

## 2) 런타임 모델

- `orca_auto run-dir`: 작업을 큐에 안전하게 등록합니다.
  - 활성 상태인 작업이 남아 있는 디렉터리는 중복 실행 방지를 위해 제출이 거부됩니다.
  - 이전 작업이 완전히 종료된 디렉터리는 새로운 실행 디렉터리(generation)를 생성하여 안전하게 재제출할 수 있습니다.
- 큐 등록 성공 시 `status: queued`를 반환하며, 제출 후 터미널을 안전하게 닫아도 백그라운드 워커가 계산을 계속 진행합니다.
- 워커가 중지되어 있는 동안 제출된 작업은 워커가 다시 시작될 때까지 큐에 안전하게 보관됩니다.
- Linux 및 WSL 환경에서 워커는 `systemd` 서비스를 통해 상주 및 재시작이 관리됩니다.
...
- `****ORCA TERMINATED NORMALLY****`가 존재
- 정상 종료했더라도 마지막 명시적 수렴 판정이 미수렴이면 완료가 아닙니다.
  마지막 판정 줄이 우선하며 analyzer·파싱 결과·진행/보고서 카드가 같은 규칙을 사용합니다.
