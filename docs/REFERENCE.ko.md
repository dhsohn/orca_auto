...
- `orca_auto service status`는 배포가 선언한 버전도 게이트합니다. editable install은
  설치 시점에 메타데이터가 동결되므로 체크아웃이 앞서 나가도 설치 당시 버전을 계속
  보고합니다. 이 명령은 그 불일치를 `version_drift`로 보고하고 `pip install -e .`
  힌트와 함께 stderr에 출력하며, 0이 아닌 코드로 종료합니다.
