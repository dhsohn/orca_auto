...
모든 unit에 해당 경로를 렌더링합니다. `--worker-only`는 full runtime target 대신
engine-worker target만 활성화합니다 — 나머지 런타임 없이 큐 작업만 실행할 머신에
사용하며, 그런 설치는 `service status`가 `worker-only`로 보고합니다.
