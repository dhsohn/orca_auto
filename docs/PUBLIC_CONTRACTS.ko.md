...
완료로 판정하려면 해당 ORCA 실행의 종료 근거가 서로 일치해야 합니다.
실제 오류 종료 진단이 있으면 정상 종료 배너가 앞이나 뒤에 있어도 `completed`로
판정하지 않습니다. 일반 종료 오류는 `unknown_failure` / `error_termination`으로
분류하며, 더 구체적인 실패는 기존 분류를 유지합니다. 종료 마커를 찾을 때는
입력 에코 줄(`| n> ...`)과 원시 주석 줄을 제외합니다.

실행한 attempt의 출력이 나머지 완료 조건을 충족하더라도 종료 코드가 0이 아니면
`unknown_failure` / `nonzero_exit_code`로 판정합니다. 원래 attempt `return_code`는
보존하며, 이미 판정한 실패 분류는 대체하지 않습니다. 출력 파일만 읽는 경로에서는
프로세스 종료 코드를 확인할 수 없습니다.
...
- 각 워커는 시작할 때 resolve한 `orca_auto` module source를 자기 process environment에
  ...
  최신 HEAD reflog 갱신 시각과 unit 시작 시각을 대조합니다. 현재 커밋을 이름하는 모든
  HEAD reflog 항목을 갱신으로 셉니다. 같은 커밋으로의 checkout·reset도 포함하는데, 강제
  checkout은 no-op checkout과 같은 subject를 쓰면서 파일을 되돌리므로 판정은 stale 쪽으로
  치웁니다. 커밋 객체 시각, status 명령
...
- `service restart`는 재시작할 워커 서비스 각각의 start-limit 실패 상태를 지웁니다. runtime
  ...
  0이 아닌 코드로 종료합니다.
- `service restart`는 서비스를 변경하기 전에 대상 워커들의 공유 admission pool에
  실행 중이거나 예약된 계산이 없는지 확인합니다. 호출한 셸의 설정이 아니라 설치된 서비스의
  설정을 사용하며, target과 모든 워커의 재시작이 끝날 때까지 기존 admission lock을 유지해
  확인 직후 새 계산이 해당 pool에 진입하지 못하게 합니다. ORCA·xTB·CREST 슬롯을 모두
  세며, 아직 슬롯을 받지 않은 대기 작업은 차단 사유가 아닙니다. admission 기록을 정리하거나
  복구하지 않습니다.
- 설정·프로세스 신원을 확인할 수 없거나 실행 후 설정이 변경된 경우, admission 디렉터리나
  lock이 없거나 상태가 손상되어 읽을 수 없는 경우, lock을 얻지 못한 경우에는 이유를 안내하고
  0이 아닌 코드로 기본 재시작을 거부합니다. guard는 installer가 생성한 워커 명령과 절대 경로의
  일반 설정 파일을 대상으로 하며, 알 수 없는 명령·환경 override를 추측하지 않습니다.
  프로세스 신원과 설정 파일 시각을 검사하지만 과거 메모리의 설정을 복원하지는 않습니다.
  실행 중 설정·경로 변경, 시스템 시계 변경, 보존·소급된 파일 시각은 이 guard의 보장 밖입니다.
  실행 중인 워커의 설정은 변경하지 마세요.
- `service restart --force`는 유휴 검사를 생략하고, 실행 중인 계산의 중단 및 recovery 횟수
  소모 가능성을 경고합니다. 기존 unit 선택·권한 검사는 유지합니다. 자동 drain, 유휴 대기,
  예약 재시작 기능은 없으며, 계산 슬롯을 쓰지 않는 workflow orchestration cycle의 무중단도
  보장하지 않습니다. 가능한 한 유휴 창에서 재시작하세요.
...
- `src/orca_auto`와 `extensions/workflows/src/orca_auto/flow` 아래 private Python 함수와 helper 모듈.
- 내부 worker child 명령줄.
