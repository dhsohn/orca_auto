...
컨포머 스크리닝을 포함합니다. 두 프로젝트는 `6.0.0`을 사용하며,
정확히 같은 버전이 필요합니다.
...
루트의 `workflows` extra는 동일 버전 확장을 고정합니다. PyPI의 릴리스 패키지 대신
로컬 확장 소스를 사용하도록 위 명령에 두 로컬 프로젝트를 명시하세요. 안정 버전은
PyPI와 GitHub Releases에서 제공하며, 과거 5.0.0 배포물은 GitHub Releases에만 있습니다.
