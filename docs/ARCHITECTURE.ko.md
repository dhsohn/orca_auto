...
1. **내구성 있는 큐 기반 실행 (Durable Queueing)**:
   사용자가 작업을 제출하면 즉시 디스크에 원자적으로 영속화되며, 터미널 종료나 시스템 재부팅 후에도 작업이 유실되지 않습니다.
2. **독립된 실행 회차 격리 (Generation Isolation)**:
   동일한 작업 디렉터리에 재제출하더라도 이전 실행 기록을 덮어쓰지 않고 새로운 실행 회차(generation) 디렉터리에 분리하여 저장합니다.
3. **무분별한 자동 재시도 방지 (Explicit Recovery)**:
   화학 계산 실패 시 원본 입력을 임의로 수정하거나 자원을 낭비하며 무한 재시도하지 않고, 명확한 실패 진단과 기록을 남깁니다.
4. **명확한 데이터 기준 (Source of Truth)**:
   모든 상태와 결과의 최종 기준은 디스크에 저장된 영속 JSON 파일(`job_state.json`, `queue.json`)입니다. 빠른 조회를 위한 SQLite 인덱스는 이 파일들로부터 언제든 무결하게 재생성할 수 있습니다.
...
| **`orca/`** | ORCA 입력(`.inp`) 파싱 및 자원 판별, 실행 준비, 출력 로그 해석 및 수렴 판정, 결과 보고서(`machine.json`) 발행 |
...
- 입력 파일 및 종속 파일의 스냅샷을 구성하고, 큐에 작업을 원자적으로 등록한 후 CLI는 즉시 반환됩니다.
...
- 자원이 충분하면 슬롯을 예약하고 독립 실행 회차 디렉터리를 생성하여 ORCA를 구동합니다. 만약 가용 메모리가 일시적으로 부족하면 작업을 실패 처리하지 않고 자원 대기(`waiting for resources`) 상태로 큐에 안전하게 유지합니다.
...
### ④ 상태 확정 및 결과 발행 (Publication)
- ORCA 계산이 끝나면 `orca/out_analyzer.py`가 출력 파일의 실제 종료 문자열 및 에너지 수렴 여부를 분석합니다. (입력 echo나 주석에 포함된 오류 문구에 오작동하지 않도록 엄격히 검증)
- 검증된 과학적 데이터(에너지, 수렴 여부, 열역학 데이터 등)를 바탕으로 다운스트림 도구 연동을 위한 표준 `machine.json`(v1 Envelope 규격) 및 HTML 요약본을 발행합니다.
...
- **과거 데이터 보호**: 7.0에서 은퇴한 이전 워크플로우 디렉터리는 과거 계산 데이터를 보존하기 위해 읽기 전용으로 보호되며, 해당 디렉터리에서 새로운 실행이 시작되는 것을 방지합니다.
