...
이 문서는 ORCA_auto의 내부 아키텍처와 작업 실행 라이프사이클을 설명합니다. 패키지 구조, 큐/워커 라이프사이클, 엔진 추상화 계층, 워크플로우 오케스트레이션의 동작 방식을 다룹니다.

CLI 및 명령어 사용법은 [빠른 시작](QUICKSTART.ko.md)과 [레퍼런스](REFERENCE.ko.md)를, 패키지 및 개발 규칙은 [개발 가이드](DEVELOPMENT.ko.md)를 참고하세요.

---

## 1. 개요 및 설계 원칙

ORCA_auto는 Linux 및 WSL 환경에서 ORCA 양자화학 계산을 안전하게 자동화하고 다단계 워크플로우를 조율하는 **큐 기반 작업 실행기(Queue-first Runner)**입니다.

핵심 아키텍처 원칙:

- **안전한 큐 등록 및 비동기 워커 실행**: CLI(`run-dir`)는 계산을 포그라운드에서 직접 수행하지 않고, 입력 검증 후 디스크 큐(`queue.json`)에 등록한 뒤 즉시 종료합니다.
- **systemd 기반 워커 감독**: 백그라운드에서 상주하는 워커 데몬이 큐에 쌓인 작업을 순차적으로 가져와 실행합니다.
- **투명한 파일 기반 상태 추적**: 작업 상태와 최종 보고서, 아티팩트는 각 계산 디렉터리 내에 구조화된 JSON(`machine.json`) 및 Markdown 형태로 기록됩니다.

ORCA는 가장 핵심적인 계산 엔진으로 동작하며, xTB와 CREST는 컨포머 탐색 등 다단계 워크플로우를 보조하는 내부 스테이지 엔진으로 활용됩니다.
